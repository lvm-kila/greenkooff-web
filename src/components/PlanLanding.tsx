...
import { PLAN_FAQS, Plan, WHATSAPP_URL } from "../content/siteContent";
    ...
    <main className="bg-white text-slate-900">
      <section className="section-shell pb-12">
        <div className="mx-auto max-w-5xl px-4 md:px-8">
          ...
          <div className="mt-6 rounded-3xl border border-blue-100 bg-gradient-to-r from-blue-800 to-blue-600 p-8 text-white">
            <p className="text-sm uppercase tracking-[0.16em] text-blue-100">{plan.name}</p>
            <h1 className="mt-2 text-4xl font-bold">{plan.subtitle}</h1>
            <p className="mt-4 max-w-2xl text-blue-100">{plan.audience}</p>
            <p className="mt-6 text-3xl font-bold">{plan.price}</p>
            <div className="mt-6 flex flex-wrap gap-3">
              <a href="#contratar" className="rounded-xl bg-orange-500 px-6 py-3 font-semibold text-white transition hover:bg-orange-600">
                Contratar
              </a>
          ...
          <article className="card-premium">
            ...
            <ul className="mt-5 grid gap-3 md:grid-cols-2">
              {plan.includes.map((item) => (
                <li key={item} className="flex items-start gap-2 rounded-xl border border-blue-100 bg-blue-50/60 p-3 text-slate-700">
          ...
          <article className="card-premium">
            ...
            <div className="mt-5 space-y-3">
              {PLAN_FAQS[plan.slug].map((faq) => (
                <details key={faq.question} className="rounded-xl border border-blue-100 p-4">
